...
class LesionDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        # self.transform_C = get_transform(self.opt)
    ...
    def __getitem__(self, index):
        ...
        return {'A': A, 'B': B, 'A_paths': A_path, 'B_paths': B_path} # yb    
    ...
    def augment(self, img, dataset, phase="train"):
        ...
        if phase == "train":
            if dataset=="A" or dataset=="B":
                ...
                img = np.rot90(img)
                img = np.fliplr(img)  # yb
            if dataset=="C":
                img = np.rot90(img, k=1) #yb
                # img = self.smooth(img, sigma=2)
            # img = self._fliplr(img) # flip left-right
